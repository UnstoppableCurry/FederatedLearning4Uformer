...
class Train_client(Thread):
    def __init__(self, obj, train_dataset, net, criterion, global_parameters, tb_writer, clent_name, clent_drive,nums):
    ...
    def run(self):
        self.result = self.obj.localUpdate(self.train_dataset, self.net, self.criterion, self.global_parameters,
                                           self.tb_writer, self.clent_name, self.clent_drive,self.nums)
...
if __name__ == "__main__":
    ...
    for i in range(opt.num_comm):
        ...
        if opt.drop_out==0:#默认策略
            ...
            order = np.random.permutation(opt.num_of_clients)
            clients_in_comm = ['client{}'.format(i) for i in order[0:num_in_comm]]
            for drop_out_client_number in range(opt.drop_out_client):
                clients_in_comm=np.delete(clients_in_comm,0,0)
        elif opt.drop_out==2:
            # 对随机选的将100个客户端进行随机排序
            order = np.random.permutation(opt.num_of_clients)
            for drop_out_client_number in range(opt.drop_out_client):
        ...
        thds = []
        my_split_Data_historoy=myClients.split_Data_historoy
        all_client_data_nums=0
        for client_name_index in range(len(clients_in_comm)):
            ...
            thd1 = Train_client(myClients.clients_set[client_name],
                                ...
                                'cuda:' + str(client_name_index),nums=my_split_Data_historoy[order[0:num_in_comm][client_name_index]])
            all_client_data_nums+=my_split_Data_historoy[order[0:num_in_comm][client_name_index]]
            thds.append(thd1)
        for thd in thds:
            thd.start()
        for thd in thds:
            thd.join()
        for thd in thds:
            all_local_parameters.append(thd.get_result())

        if opt.sr_weight==1:
            for thd_local_parameters in all_local_parameters:
                thd_local_parameters=thd_local_parameters[0]
        ...
        elif opt.sr_weight==0:
            for all_local_parameter in all_local_parameters:  # 读取每个client权重
                thd_local_parameters = all_local_parameter[0]  # client 权重
                data_len_num = all_local_parameter[1]  # client 数据集数量 all_client_data_num本次通讯数据集总数量
                print('data_len_num', data_len_num)
                if sum_parameters is None:
                    sum_parameters = {}
                    for key, var in thd_local_parameters.items():
                        # sum_parameters[key] = var.clone()
                        sum_parameters[key] = data_len_num * var.clone() / all_client_data_nums  # 初始化分配 按照设定比例进行存

                else:
                    for var in sum_parameters:
                        sum_parameters[var] = sum_parameters[var] + thd_local_parameters[
                            var] * data_len_num / all_client_data_nums  # 每个client模型中的每一层都x client数据集数量/本次通讯数据集总数量
        ...
        with torch.no_grad():
            ...
            for ii, data_val in enumerate(val_loader, 0):
                ...
                with torch.cuda.amp.autocast():  # 给用户提供了较为方便的混合精度训练机制
                    ...
                    if tb_writer:
                        ...
                        tb_writer.add_scalar("serve_eval_each_MSE_loss", loss.cpu().numpy(), steps)
                        tb_writer.add_scalar("serve_eval_each_PSNR_loss", batch_PSNR, steps)
                        tb_writer.add_scalar("serve_eval_each_MAE_loss", MAE, steps)
            psnr_val_rgb = np.array(psnr_val_rgb).mean()
            MAE_val_rgb = np.array(MAE_val_rgb).mean()
            PSNR_val_rgb = np.array(PSNR_val_rgb).mean()
            # psnr_val_rgb = sum(psnr_val_rgb) / (len_valset//opt.batch_size)
            # MAE_val_rgb = sum(MAE_val_rgb) / (len_valset//opt.batch_size)
            # PSNR_val_rgb = sum(PSNR_val_rgb) / (len_valset//opt.batch_size)
            if tb_writer:
            ...
            torch.save({'epoch': 0,
                        ...
                        'optimizer': 0
                                         ...
                                         '{}_num_comm{}_E{}_B{}_lr{}_num_clients{}_cf{}.pth'.format(opt.model_name,
                                                                                                   i, opt.nepoch,
                                                                                                   opt.batch_size,
                                                                                                   opt.lr_initial,
                                                                                                   opt.num_of_clients,
                                                                                                   opt.cfraction)))
            # torch.save(net.state_dict(), os.path.join(opt.save_path,
            #                              '{}_num_comm{}_E{}_B{}_lr{}_num_clients{}_cf{}.pth'.format(opt.model_name,
            #                                                                                        i, opt.nepoch,
            #                                                                                        opt.batch_size,
            #                                                                                        opt.lr_initial,
