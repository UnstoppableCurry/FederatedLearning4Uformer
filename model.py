...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        H = int(math.sqrt(L))#math.sqrt: 返回数字x的平方根
        W = int(math.sqrt(L))
        x = x.transpose(1, 2).contiguous().view(B, C, H, W)
        out = self.conv(x).flatten(2).transpose(1,2).contiguous()  # B H*W C  flatten是numpy.ndarray.flatten的一个函数，即返回一个折叠成一维的数组。
        return out
...
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None,act_layer=nn.LeakyReLU):
        ...
        # )
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, out_channel, kernel_size=3, stride=stride, padding=kernel_size//2),
            act_layer(inplace=True)
        )
...
class OutputProj(nn.Module):
    def __init__(self, in_channel=64, out_channel=2, kernel_size=3, stride=1, norm_layer=None,act_layer=None):
        super().__init__()
        # self.proj = nn.Sequential(
        #     nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=kernel_size//2),
        # )
        self.proj = nn.Sequential(
            ...
            # nn.Conv2d(64, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, 64, kernel_size=3, stride=stride, padding=kernel_size//2),
            nn.Conv2d(64, 2, kernel_size=3, stride=stride, padding=kernel_size//2)
        )
...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=5,
    ...
    def forward(self, x, mask=None):
        # Input Projection
        # print('input device',x.device,mask.device)
        # print('model device',self.input_proj.proj[0].weight.device)
        y = self.input_proj(x)
        y = self.pos_drop(y)
        #Encoder
...
class Uformer_CatCross(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        # Input/Output
        self.input_proj = InputProj(in_channel=in_chans, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=embed_dim, out_channel=2, kernel_size=3, stride=1)
